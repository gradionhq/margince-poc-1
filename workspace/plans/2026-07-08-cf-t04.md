...
- [x] **Step 4: Commit** → 610488fe4ae8a506cf3d468b3572d3b0e42cc7c1
...
- [x] **Step 4: Commit** → c07dea20a8fc0b330465a38cd4f3f84a9e8e3c10
...
- [x] **Step 5: Commit** → 738fb760062f51628aab51766caecfc0ce3b28a5
...
- [x] **Step 5: Commit** → 623dbc88bd37d52ab0bb98f811a36ee2d3dc689f
...
- [x] **Step 5: Commit** → d978eb68f5afae65c0aeccfc59ffa4011cc80849
...
- [x] **Step 7: Commit** → 7d06075ddcec8e9b50b048e849f77ab33cb19ad7
...
- [x] **Step 4: Commit** → 0cc4bb9962965428f315e14829aba8ed4db6d7cb
...
- [x] **Step 5: Commit** → f51128b78289ceabd0052e0e22d0164ed4c9f11e
...
CF-T04 wires `renameCustomField` (🟢) and `retireCustomField` (🟡) — previously 501 stubs — through
the governed add-field engine at `backend/internal/platform/customfields/`, and adds new contract
surface `PATCH /custom-fields/{id}/options` (🟡) for picklist option editing with CHECK-constraint
regeneration. All three write exactly one `audit_log` row (`action: "update"`, no new migration —
the existing CHECK already allows it) and none of them ever drop a physical column.

Implementer policy: `solo-claude` (one monolithic dispatch spanning all 8 plan tasks, per the
spec's `implementers: solo-claude` frontmatter). Model tier: `claude-haiku-4-5` throughout (the
CLI's default for a `claude` dispatch; no per-task tier override was needed — every task was
mechanical-to-moderate transcription against an already-detailed plan).

Three post-implementation fixes landed during the review/UAT loop (see Escalations — none were
plan-authoring failures on the implementer's part; two were coordinator-plan gaps, one a
pre-existing seed gap this ticket's new endpoints exposed):
1. A forked duplicate of `httpkit.PathID` in `handler.go` → replaced with the shared helper
   (code-review finding).
2. The contract change's regenerated frontend TS types (`crm.d.ts`) were never committed — the
   coordinator's own plan omitted that file from Task 1's `git add` list (coordinator-authoring
   gap, fixed directly).
3. The `admin` seed role's `custom_field` RBAC permission lacked `update`, so the two new
   PATCH-gated endpoints 403'd before ever reaching the approval-token/toolgate logic
   (live-stack UAT finding) — `backend/seed/dev.sql` now grants `update` alongside the existing
   `read`/`create`.
...
|---|------|-------|------|----------|--------|-------|
| 1 | Contract — add updateCustomFieldOptions | claude (solo dispatch) | haiku | no (whole-plan dispatch) | done | `610488f` |
| 2 | Engine — extract checkConstraintClause + BuildOptionsDDL + Created.ArchivedAt | claude (solo dispatch) | haiku | no | done | `c07dea2` |
| 3 | Engine — Rename | claude (solo dispatch) | haiku | no | done | `738fb76` |
| 4 | Engine — Retire | claude (solo dispatch) | haiku | no | done | `623dbc8` |
| 5 | Engine — SetOptions | claude (solo dispatch) | haiku | no | done | `d978eb6` |
| 6 | Handler — wire ServeHTTP suffix-route dispatch | claude (solo dispatch) | haiku | no | done | `7d06075` |
| 7 | Adapter — delegate CustomFieldsAdapter | claude (solo dispatch) | haiku | no | done | `0cc4bb9` |
| 8 | UAT guide + final verification gate | claude (solo dispatch) | haiku | no | done | `f51128b` |

Post-implementation review-driven fixes (signed, un-trailered, not plan tasks):
`8121bf4` (dedupe pathID → httpkit.PathID), `72c46ee` (commit missing generated TS types),
`3fce630` (add missing options-edit 404 HTTP test), `66fcb2d` (seed: grant admin `update` on
`custom_field`).

## Test results

`make check-q`: PASS (backend fmt/lint/invariants/unit tests + frontend tsc/DS-purity/unit tests).
`make gen-types-check`, `make gen-mcp-tools-check`, `make audit-coherence`, `make arch-lint`: all
PASS. `go test -tags integration ./internal/platform/customfields/...`: PASS, 0 skips (rename,
retire, options-edit: happy path, not-found, validation, agent-token flows, audit-row counts,
CHECK-regeneration proof, column/data-preservation proof). `make test-integration` (full lane, live
stack): PASS, 0 skips across 29 packages.

## Code review

`code-reviewer` (final gate, full `merge-base(main,HEAD)..HEAD` diff): **GO**. Every acceptance
criterion (CUSTOM-FIELDS-WIRE-3/4, CUSTOM-FIELDS-PARAM-5, CUSTOM-FIELDS-AC-13) verified against the
diff; `.ai/checklists/code-review.md` applied. One actionable finding (the `pathID` duplicate) was
routed and fixed (`8121bf4`); one cosmetic finding (an unreachable generic message string on
green-tier rename's dead approval-required branch) was left as-is — genuinely unreachable code path,
not worth a fix commit.

`swarm-reviewer` (STATE=FINAL-UAT, coverage): first pass **BLOCK** — the nonexistent-id → 4xx UAT
assertion had no HTTP-level test for the options-edit route specifically (engine-level coverage
only). Fixed (`3fce630`); re-check **PASS**.

## Craftsmanship

Not yet run — scheduled for the DONE step (branch-finish taste review, `craftsmanship-reviewer`),
per the swarm-coordinator lifecycle (§7, "the only place taste review runs").

## Escalations

None open. Three issues surfaced and were resolved in-flight (not escalated to the user, per the
coordinator's "drain findings" discipline — each was a concrete, minimal, verifiable fix):

1. **Forked duplicate helper** (code-review finding, non-blocking) — `handler.go` defined a local
   `pathID` byte-identical to `httpkit.PathID`. Fixed in `8121bf4`.
2. **Coordinator plan-authoring gap** — Task 1's commit instructions omitted
   `frontend/src/lib/api-client/generated/crm.d.ts` from the `git add` list, so the TS half of the
   contract regen was silently left uncommitted (working-tree-only) through the rest of the
   implementer's dispatch. Caught during the coordinator's post-dispatch light review (a `git
   status` on the worktree showed it dirty), fixed in `72c46ee`.
3. **Pre-existing seed gap, newly exposed** — `backend/seed/dev.sql`'s `admin` role never granted
   `update` on `custom_field` (the object had no PATCH-shaped operation before this ticket). Found
   by the live-stack UAT runner (both new PATCH endpoints 403'd for the seeded admin), fixed in
   `66fcb2d`. Live-stack UAT re-run confirmed PASS after the fix, artifact pinned to `66fcb2d`.

---
# Manual UAT Guide

The full runnable guide lives at `workspace/manual-test/cf-t04.md` (Task 8) — executed verbatim by
the `swarm-uat-runner` live-stack gate (PASS, pinned to head `66fcb2d`). This section maps the
spec's UAT checklist to that guide and to automated coverage.

1. **Rename a text field's label via `PATCH /custom-fields/{id}` as a human caller (no token) →
   200, `column_name` unchanged, one audit row.**
   **[auto]** `TestRenameCustomField_HumanNoToken_200_ColumnNameUnchanged` +
   `TestRename_UpdatesLabelOnly_ColumnNameStableOneAuditRow`
   (`backend/internal/platform/customfields/lifecycle_http_integration_test.go`,
   `rename_integration_test.go`). **[live]** guide § 1, steps 1-3.

2. **Retire an active field as a human caller → 200, `status: "retired"`, `archived_at: null`, data
   still queryable directly against the column; as an agent caller without a token → 403
   `approval_required`; with a valid token → 200.**
   **[auto]** `TestRetireCustomField_HumanThenAgentFlow` +
   `TestRetire_FlipsStatus_PreservesColumnAndData_ArchivedAtStaysNull` (same files as above,
   `retire_integration_test.go`). **[live]** guide § 2, all 5 steps.

3. **Edit a picklist field's options (add one, remove one, keep ≥1) → 200, inserting a row with the
   new option succeeds, inserting a row with a removed option now fails the regenerated CHECK;
   attempt to remove the last remaining option → 422 `"A picklist needs at least one option"`.**
   **[auto]** `TestSetCustomFieldOptions_AddRemoveKeepAtLeastOne` +
   `TestSetOptions_RegeneratesCheck_OneAuditRow` + `TestSetOptions_RemovingLastOption_ErrLastOption`
   (`options_integration_test.go`). **[live]** guide § 3, all 5 steps — including the direct
   `UPDATE ... SET cf_channel='reseller'` proving the regenerated CHECK actually rejects the removed
   value in the live database, not just at the Go layer.

4. **Attempt any rename/retire/options-edit against a non-picklist field for the options endpoint,
   or a nonexistent field id for all three → expected 4xx, not a 500.**
   **[auto]** `TestSetCustomFieldOptions_NonPicklistField_422`,
   `TestRenameCustomField_NonexistentID_404`, `TestRetireCustomField_NonexistentID_404`,
   `TestSetCustomFieldOptions_NonexistentID_404` (the last one added specifically to close a
   FINAL-UAT coverage gap — see Escalations). **[live]** guide § 4, all 3 steps, plus a repo-wide
   grep confirming no `DROP COLUMN` code path exists.

No step in this ticket required genuine human/visual judgment — every UAT assertion is either
API-level-testable or database-state-verifiable, so there are no **[manual]**-only steps.
