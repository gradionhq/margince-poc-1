...
// noopVerifier satisfies the approvalsport.Verifier interface for tests.
type noopVerifier struct{}

func (noopVerifier) VerifyAndConsume(_ context.Context, _ string, _ approvalsport.Binding) error {
	return nil
}
...
func buildRBACMux(t *testing.T, db *sql.DB) http.Handler {
	...
	authWrap := func(object, action string, h http.Handler) http.Handler {
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if !userCanDo(r.Context(), db, p.TenantID, p.UserID, object, action) {
				http.Error(w, `{"code":"forbidden"}`, http.StatusForbidden) //nolint:forbidigo
				return
			}
			h.ServeHTTP(w, r)
		})
	}
	personH := peopletransport.NewPersonHandler(
	...
	orgH := orgstransport.NewOrganizationHandler(
		organizations.NewOrgStore(db),
		relationships.NewRelationshipStore(db),
		deals.NewDealStore(db),
		activities.NewActivityStore(db),
		noopVerifier{},
	...
	mux.Handle("/organizations", authWrap("organization", "read", orgH))
	return mux
}
