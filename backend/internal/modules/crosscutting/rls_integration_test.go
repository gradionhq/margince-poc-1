...
func TestAuditLogImmutable(t *testing.T) {
	...
	// Migration 000019 dropped the silent DO INSTEAD NOTHING rules so
	// DELETE/UPDATE now RAISE EXCEPTION rather than silently no-op.
	if err := asTenant(t, pool, ws, func(tx pgx.Tx) error {
...
func TestOrgCyclePrevention(t *testing.T) {
	...
	// Closing the loop A -> C -> B -> A must be rejected.
	err := asTenant(t, pool, ws, func(tx pgx.Tx) error {
...
func TestDealCompositeFK(t *testing.T) {
	...
	// The trg_deal_stage_pipeline trigger must reject stage from wrong pipeline.
	err := asTenant(t, pool, ws, func(tx pgx.Tx) error {
		_, e := tx.Exec(ctx,
...
func TestRelationshipStakeholderRoleEnum(t *testing.T) {
	...
	if err := asTenant(t, pool, ws, func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`INSERT INTO organization(workspace_id,name,source,captured_by) VALUES($1,'RoleOrg','api','human:test') RETURNING id`,
			ws).Scan(&org)
	}); err != nil {
