...
import (
	...
	partadapters "github.com/gradionhq/margince/backend/internal/modules/partners/adapters"
	partdomain "github.com/gradionhq/margince/backend/internal/modules/partners/domain"
...
func TestPartnerHandler_UpsertThenGetThenList(t *testing.T) {
	...
	h := NewPartnerHandler(partadapters.NewPartnerStore(db))
	...
	var page struct {
		Data []partdomain.Partner `json:"data"`
	}
...
func TestPartnerRBAC_NonAdminDenied(t *testing.T) {
	...
	h := NewPartnerHandler(partadapters.NewPartnerStore(db))
	guarded := platformauth.RbacMiddleware(db, platformauth.ObjPartner)(h)
