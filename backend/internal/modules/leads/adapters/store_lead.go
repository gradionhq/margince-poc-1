...
func (s *LeadStore) Create(ctx context.Context, l domain.Lead) (domain.Lead, error) {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		if errors.As(err, &pgErr) && pgErr.Code == "23505" && pgErr.Constraint == "uq_lead_email" {
			...
			return domain.Lead{}, &ErrLeadEmailDuplicate{ExistingID: existingID}
		}
		return domain.Lead{}, err
	}
...
func (s *LeadStore) Get(ctx context.Context, id, workspaceID string) (domain.Lead, error) {
	var l domain.Lead
...
func (s *LeadStore) List(ctx context.Context, workspaceID, cursor string, limit int) ([]domain.Lead, string, error) {
...
func (s *LeadStore) Update(ctx context.Context, id, workspaceID string, updates map[string]any, ifMatch int64) (domain.Lead, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return domain.Lead{}, err
	}
	...
	if n, _ := res.RowsAffected(); n == 0 {
		if ifMatch != 0 {
			return domain.Lead{}, errs.ErrVersionSkew
		}
...
func (s *LeadStore) Archive(ctx context.Context, id, workspaceID string) (domain.Lead, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return domain.Lead{}, err
	}
...
//
//nolint:dupl // parallel per-entity CRUD: the SQL column list and Scan targets differ by type; a generic extraction would read worse than the explicit form
func (s *LeadStore) getAny(ctx context.Context, id, workspaceID string) (domain.Lead, error) {
	var l domain.Lead
	err := withWorkspaceTx(ctx, s.db, workspaceID, func(tx *sql.Tx) error {
		return tx.QueryRowContext(ctx, `
			SELECT id, workspace_id, full_name, email, title, company_name,
...
//  2. Sets lead status=promoted + promoted_person_id + promoted_at.
//  3. Writes an audit_log row.
//
// Returns the newly-created person's ID.
// The caller may look up the full Person record via the people module.
func (s *LeadStore) Promote(ctx context.Context, leadID, workspaceID, actorID string) (string, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return "", err
	}
	...
	if _, err := tx.ExecContext(ctx, `SELECT set_config('app.workspace_id',$1,true)`, workspaceID); err != nil {
		return "", fmt.Errorf("promote set guc: %w", err)
	}
	...
	if l.Status == domain.StatusPromoted {
		return "", errs.ErrConflict
	}

	// 2. Create person (INSERT using individual fields — no Person struct import needed)
	personID := ids.New()
	...
	if _, err := crmaudit.WriteTx(ctx, tx, crmaudit.Entry{
		...
		Action: "promote", EntityType: "lead", EntityID: &leadID,
	}); err != nil {
