...
func (s *RelationshipStore) List(ctx context.Context, workspaceID, cursor string, limit int, filter RelationshipListFilter) ([]Relationship, string, error) {
...
func (s *RelationshipStore) Update(ctx context.Context, id, workspaceID string, updates map[string]any, ifMatch int64) (Relationship, error) {
	var kind string
	...
	err := withWorkspaceTx(ctx, s.db, workspaceID, func(tx *sql.Tx) error {
		...
		rel := Relationship{Kind: kind, PersonID: personID, DealID: dealID}
		entityType, entityID := owningStream(rel)
...
func (s *RelationshipStore) Archive(ctx context.Context, id, workspaceID string) (Relationship, error) {
	err := withWorkspaceTx(ctx, s.db, workspaceID, func(tx *sql.Tx) error {
		var kind string
		var personID, dealID *string
...
func (s *RelationshipStore) getAny(ctx context.Context, id, workspaceID string) (Relationship, error) {
	var rel Relationship
	err := withWorkspaceTx(ctx, s.db, workspaceID, func(tx *sql.Tx) error {
		row := tx.QueryRowContext(ctx,
			`SELECT `+relationshipSelectCols+` FROM relationship
