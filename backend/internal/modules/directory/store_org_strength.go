...
// org_strength. Wires OrgStrength() unmodified — never reimplements the formula.
//
...
func attachOrgAggregates(
	ctx context.Context,
	tx *sql.Tx,
	strengthActivities func(context.Context, *sql.Tx, string, []string) (map[string][]StrengthActivity, error),
	workspaceID string,
	orgs []*Organization,
) error {
	...
	for _, ids := range employeesByOrg {
		for _, id := range ids {
			if !seen[id] {
				seen[id] = true
	...
	for _, o := range orgs {
		...
		var inputs []OrgStrengthInput
		for _, personID := range employeesByOrg[o.ID] {
			result := ComputeStrength(now, activitiesByPerson[personID])
			...
			r := result
			inputs = append(inputs, OrgStrengthInput{PersonID: personID, Strength: &r, LastInteraction: lastInteraction})
		}
		score, topPersonID, hasSignal := OrgStrength(inputs)
		if !hasSignal {
			o.Strength = nil
			continue
		}
		o.Strength = &OrgStrengthBlock{
			Score:         score,
