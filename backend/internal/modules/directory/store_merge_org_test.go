...
func mkOrg(ctx context.Context, t *testing.T, store *OrgStore, ws, name string) Organization {
	t.Helper()
	created, err := store.Create(ctx, Organization{WorkspaceID: ws, DisplayName: name, Source: "api", CapturedBy: "human:t"})
	if err != nil {
...
func TestOrgMergeSelfMergeRejected(t *testing.T) {
	...
	if _, err := orgStore.Merge(ctx, o.ID, o.ID, ws); !errors.Is(err, ErrSelfMerge) {
