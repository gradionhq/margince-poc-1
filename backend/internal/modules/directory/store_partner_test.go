...
func TestPartnerStore_Upsert_CreatesThenUpdatesSameRowAndSetsOrgClassification(t *testing.T) {
	...
	store := crmcore.NewPartnerStore(db)

	role := "hosting"
	p := crmcore.NewPartner(orgID, prov.Provenance{Source: "test", CapturedBy: "human:test"})
...
func TestPartnerStore_List_FiltersByRoleAndCertStatus(t *testing.T) {
	...
	pa := crmcore.NewPartner(orgA, prov.Provenance{Source: "test", CapturedBy: "human:test"})
	...
	pb := crmcore.NewPartner(orgB, prov.Provenance{Source: "test", CapturedBy: "human:test"})
	...
	items, _, err := store.List(context.Background(), partnerTestWorkspaceID, "", 20,
		crmcore.PartnerListFilter{PartnerRole: "hosting", CertStatus: "certified"})
	if err != nil {
		t.Fatalf("List: %v", err)
	}
