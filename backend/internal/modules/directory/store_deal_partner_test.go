...
func openDealPartnerTestDB(t *testing.T) *sql.DB {
	t.Helper()
...
func TestDealStore_Update_PersistsPartnerOrgIDChangeWithAudit(t *testing.T) {
	db := openDealPartnerTestDB(t)
	pipelineID, stageID, orgA, orgB := seedDealPartnerFixtures(t, db, "update")
	store := crmcore.NewDealStore(db)

	d := crmcore.NewDeal("Deal with partner", pipelineID, stageID, prov.Provenance{Source: "test", CapturedBy: "human:test"})
