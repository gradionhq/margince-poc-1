...
type orgStore interface {
	...
	List(ctx context.Context, workspaceID, cursor string, limit int, sort string, filter OrgListFilter) ([]Organization, string, error)
}
...
func (p *DatasourceProvider) Search(ctx context.Context, query datasource.SearchQuery) (datasource.SearchResult, error) {
	...
	case datasource.EntityOrganization:
		list, _, err := p.orgs.List(ctx, p.workspaceID, "", limit, "", OrgListFilter{})
		if err != nil {
			return datasource.SearchResult{}, err
		}
		for _, item := range list {
