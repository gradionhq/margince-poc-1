...
func TestOrganizationHandler_Get_Composite360(t *testing.T) {
	...
	org, err := orgStore.Create(ctx, crmcore.Organization{WorkspaceID: orgCompositeWS, DisplayName: "Composite Org", Source: p0.Source, CapturedBy: p0.CapturedBy})
	if err != nil {
	...
	if _, err := relStore.Create(ctx, crmcore.Relationship{WorkspaceID: orgCompositeWS, Kind: "employment", PersonID: &person.ID, OrganizationID: &org.ID, Source: p0.Source, CapturedBy: p0.CapturedBy}); err != nil {
	...
	d := crmcore.NewDeal("Composite Deal", pl.ID, st.ID, p0)
	...
	act, err := activityStore.Create(ctx, crmcore.Activity{WorkspaceID: orgCompositeWS, Kind: "call", OccurredAt: time.Now(), Source: p0.Source, CapturedBy: p0.CapturedBy})
	if err != nil {
	...
	otherOrg, err := orgStore.Create(ctx, crmcore.Organization{WorkspaceID: orgCompositeWS, DisplayName: "Other Org", Source: p0.Source, CapturedBy: p0.CapturedBy})
	if err != nil {
	...
	if _, err := relStore.Create(ctx, crmcore.Relationship{WorkspaceID: orgCompositeWS, Kind: "employment", PersonID: &otherPerson.ID, OrganizationID: &otherOrg.ID, Source: p0.Source, CapturedBy: p0.CapturedBy}); err != nil {
...
func TestOrganizationHandler_Get_EmptyCompositeShowsEmptyArrays_NotNull(t *testing.T) {
	...
	org, err := orgStore.Create(ctx, crmcore.Organization{WorkspaceID: orgCompositeWS, DisplayName: "Lonely Org", Source: p0.Source, CapturedBy: p0.CapturedBy})
	if err != nil {
		t.Fatalf("seed org: %v", err)
	}
...
func TestOrganizationHandler_Get_ArchivedStillFetchableWithComposite(t *testing.T) {
	...
	org, err := orgStore.Create(ctx, crmcore.Organization{WorkspaceID: orgCompositeWS, DisplayName: "Archive Me Org", Source: p0.Source, CapturedBy: p0.CapturedBy})
	if err != nil {
		t.Fatalf("seed: %v", err)
	}
...
func TestOrganizationHandler_Get_Composite360_PerfBudgetAndPaginationCap(t *testing.T) {
	...
	org, err := orgStore.Create(ctx, crmcore.Organization{WorkspaceID: ws, DisplayName: "Perf Org", Source: p0.Source, CapturedBy: p0.CapturedBy})
	if err != nil {
		t.Fatalf("seed org: %v", err)
	}
	...
	for i := 0; i < 60; i++ {
		...
		if _, err := relStore.Create(ctx, crmcore.Relationship{WorkspaceID: ws, Kind: "employment", PersonID: &person.ID, OrganizationID: &org.ID, Source: p0.Source, CapturedBy: p0.CapturedBy}); err != nil {
