// Package transport holds the relationship module's HTTP handler for
// /relationships and /relationships/{id}. It mirrors handler_deal.go's
// package layout and minimal-duplication convention: the small HTTP helpers
// below intentionally reuse the shared transport helpers rather than export
// anything solely for this handler.
package transport
...
const (
	...
	// across the package boundary. handler_deal.go does not declare them.
	fieldKind     = "kind"
	fieldPersonID = "person_id"
)

type relationshipStoreSeam interface {
	Create(ctx context.Context, rel directory.Relationship) (directory.Relationship, error)
	...
	Update(ctx context.Context, id, workspaceID string, updates map[string]any, ifMatch int64) (directory.Relationship, error)
	Archive(ctx context.Context, id, workspaceID string) (directory.Relationship, error)
}
...
func NewRelationshipHandler(store *directory.RelationshipStore) *RelationshipHandler {
...
// header and the store treats an already-archived row as a no-op, matching the
// existing archive precedent in the transport package.
