...
func TestOrgStore_Restore_ArchivedOrgWritesEventAndAudit(t *testing.T) {
	...
	created, err := store.Create(orgRestoreCtx(), crmcore.Organization{
...
func TestOrgStore_Restore_RefusesLiveAndMergedRecords(t *testing.T) {
	...
	live, err := store.Create(orgRestoreCtx(), crmcore.Organization{
		WorkspaceID: orgRestoreWS,
	...
	merged, err := store.Create(orgRestoreCtx(), crmcore.Organization{
		WorkspaceID: orgRestoreWS,
