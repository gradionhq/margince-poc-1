...
func TestHistoryHandler_AC1_ActorAndAuthorityRender(t *testing.T) {
	...
	reader := NewAuditHistoryReader(db)
	h := NewHistoryHandler(reader, noopAuthz)
	...
	var agentEntry *AuditHistoryEntry
...
func TestHistoryHandler_AC2_FieldMaskingAndObjectLevelDeny(t *testing.T) {
	...
	masks := map[string]EntityFieldMask{
		entityType: {"salary": struct{}{}},
	}
	reader := NewAuditHistoryReader(db).withFieldMasks(masks)
	h := NewHistoryHandler(reader, noopAuthz)
	...
	hDeny := NewHistoryHandler(reader, denyAuthz)
