...
func (s *ActivityStore) Create(ctx context.Context, a Activity) (Activity, error) {
	if err := requireProvenance(a.Source, a.CapturedBy); err != nil {
		return Activity{}, err
	}
	a.ID = ids.New()
	err := withWorkspaceTx(ctx, s.db, a.WorkspaceID, func(tx *sql.Tx) error {
...
func (s *ActivityStore) Get(ctx context.Context, id, workspaceID string) (Activity, error) {
	var a Activity
...
func (s *ActivityStore) List(ctx context.Context, workspaceID, entityType, entityID, cursor string, limit int) ([]Activity, string, error) {
...
func (s *ActivityStore) Update(ctx context.Context, id, workspaceID string, updates map[string]any, ifMatch int64) (Activity, error) {
...
func (s *ActivityStore) Archive(ctx context.Context, id, workspaceID string) (Activity, error) {
	err := withWorkspaceTx(ctx, s.db, workspaceID, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx,
...
func (s *ActivityStore) getAny(ctx context.Context, id, workspaceID string) (Activity, error) {
	var a Activity
	err := withWorkspaceTx(ctx, s.db, workspaceID, func(tx *sql.Tx) error {
		return tx.QueryRowContext(ctx, `
			SELECT id, workspace_id, kind, subject, body,
