...
// handler_org_merge_test.go — ported from modules/directory/transport/handler_org_merge_test.go
// (package transport; crmcore imports replaced with organizations/adapters and
// organizations/domain; handler constructor updated for approvalsport.Verifier;
// seedWorkspace and seedAppUser added as local helpers since this transport
// package has no shared helper file).
package transport
...
// seedMergeWorkspace seeds a workspace row for the merge handler tests.
// Uses a dynamic wsID per-test run so concurrent tests don't collide.
func seedMergeWorkspace(t *testing.T, db *sql.DB, wsID string) {
	t.Helper()
...
// seedMergeAppUser seeds an app_user row so that audit_log.on_behalf_of FK is
// satisfied when agent-principal requests invoke crmaudit.Write.
func seedMergeAppUser(t *testing.T, db *sql.DB, id, wsID string) {
	t.Helper()
	if _, err := db.Exec(
...
func createTestOrg(t *testing.T, store *orgAdapters.OrgStore, ws, name string) orgDomain.Organization {
	t.Helper()
...
func TestMergeOrgAgentRequiresApprovalToken(t *testing.T) {
	...
	target := createTestOrg(t, store, ws, "Target Co")
	agentUserID := ids.New()
	seedMergeAppUser(t, db, agentUserID, ws)
