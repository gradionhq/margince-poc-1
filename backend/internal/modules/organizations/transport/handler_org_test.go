...
// handler_org_test.go — ported from modules/directory/transport/handler_org_test.go
// (package transport; crmcore imports replaced with organizations/adapters,
// organizations/domain, and module-level stores; handler constructor updated
// for new approvalsport.Verifier parameter).
package transport
...
func orgHandlerForTest(db *sql.DB, store *orgAdapters.OrgStore) *OrganizationHandler {
	return NewOrganizationHandler(
		store,
...
func TestOrganizationHandler_List_WithAggregates(t *testing.T) {
	...
	// Insert person via raw SQL (no full PersonStore in organizations package).
	var personID string
	if err := db.QueryRow(
		`INSERT INTO person (id, workspace_id, full_name, source, captured_by, version)
		 VALUES (uuidv7(), $1::uuid, $2, 'test', 'human:test', 1) RETURNING id`,
		orgHandlerTestWS, "Agent-"+ids.New(),
	...
	if _, err := db.Exec(
		...
		orgHandlerTestWS, personID, org.ID,
	); err != nil {
...
func TestOrganizationHandler_List_DomainAndOwnerFilter(t *testing.T) {
	...
	ctx := crmctx.With(context.Background(), crmctx.Principal{TenantID: orgHandlerTestWS, UserID: "human:test"})
	orgStore := orgAdapters.NewOrgStore(db)
	owned, err := orgStore.Create(ctx, orgDomain.Organization{
