...
// handler_org_restore_test.go — ported from modules/directory/transport/handler_org_restore_test.go
...
func TestOrganizationHandler_Restore_RefusesMergedRecord(t *testing.T) {
	...
	survivor, err := orgStore.Create(ctx, orgDomain.Organization{
		WorkspaceID: orgRestoreHandlerTestWS,
	...
	merged, err := orgStore.Create(ctx, orgDomain.Organization{
		WorkspaceID: orgRestoreHandlerTestWS,
