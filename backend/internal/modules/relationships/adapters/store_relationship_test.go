...
func seedRelPersonOrg(t *testing.T, db *sql.DB) (personID, orgID string) {
	...
	p, err := ps.Create(ctx, peopledomain.Person{
	...
	o, err := os.Create(ctx, orgdomain.Organization{
		WorkspaceID:    wsRelStore,
...
func TestRelationshipStore_CreateEmployment_ThenGet(t *testing.T) {
	...
	created, err := s.Create(ctx, domain.Relationship{
		WorkspaceID:      wsRelStore,
		Kind:             "employment",
		PersonID:         &personID,
...
func TestRelationshipStore_CreateEmployment_NullProvenanceRejected(t *testing.T) {
	...
	s := adapters.NewRelationshipStore(db)
	_, err := s.Create(ctx, domain.Relationship{
		WorkspaceID:    wsRelStore,
		Kind:           "employment",
		PersonID:       &personID,
		OrganizationID: &orgID,
...
func TestRelationshipStore_SecondCurrentPrimaryEmployment_Conflicts(t *testing.T) {
	...
	first, err := s.Create(ctx, domain.Relationship{
		WorkspaceID:      wsRelStore,
		Kind:             "employment",
		PersonID:         &personID,
		OrganizationID:   &orgA,
...
func TestRelationshipStore_List_FiltersByKindAndOrg(t *testing.T) {
	...
	items, _, err := s.List(ctx, wsRelStore, "", 20, domain.RelationshipListFilter{
		Kind:           "employment",
		OrganizationID: orgID,
	})
	if err != nil {
...
func TestRelationshipStore_Archive_ExcludedFromDefaultList_VisibleWithIncludeArchived(t *testing.T) {
	...
	live, _, err := s.List(ctx, wsRelStore, "", 20, domain.RelationshipListFilter{PersonID: personID})
	if err != nil {
	...
	withArchived, _, err := s.List(ctx, wsRelStore, "", 20, domain.RelationshipListFilter{
		PersonID:        personID,
