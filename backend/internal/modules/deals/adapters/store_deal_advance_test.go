...
func TestDealStore_Advance_OpenToWon_SingleWriteEachTable(t *testing.T) {
	...
	d := domain.NewDeal("Deal o2w", pipelineID, openA, prov.Provenance{Source: "test", CapturedBy: "human:test"})
	...
	updated, err := store.Advance(ctx, created.ID, advanceTestWorkspaceID, domain.AdvanceInput{
...
func TestDealStore_Advance_OpenToWon_BaseCurrencyNoFXRateRow_DefaultsToOne(t *testing.T) {
	...
	d := domain.NewDeal("Deal basecur-nofx", pipelineID, openA, prov.Provenance{Source: "test", CapturedBy: "human:test"})
	...
	updated, err := store.Advance(ctx, created.ID, basecurNoFXWorkspaceID, domain.AdvanceInput{
		ToStageID: wonA, Status: "won",
	}, 0, "human:test")
	if err != nil {
...
func TestDealStore_Advance_StatusMismatchRejected(t *testing.T) {
	...
	_, err = store.Advance(ctx, created.ID, advanceTestWorkspaceID, domain.AdvanceInput{
...
func TestDealStore_Advance_Reopen_ClearsClosedAtLostReasonAndFX(t *testing.T) {
	...
	amount := int64(10000)
	currency := "EUR"
	d := domain.NewDeal("Deal reopen", pipelineID, openA, prov.Provenance{Source: "test", CapturedBy: "human:test"})
	d.WorkspaceID = advanceTestWorkspaceID
	...
	reason := "budget cut"
	closed, err := store.Advance(ctx, created.ID, advanceTestWorkspaceID, domain.AdvanceInput{
	...
	reopened, err := store.Advance(ctx, created.ID, advanceTestWorkspaceID, domain.AdvanceInput{
		ToStageID: openA,
	}, 0, "human:test")
	if err != nil {
