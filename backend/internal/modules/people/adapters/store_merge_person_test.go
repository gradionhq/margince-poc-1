...
func mkPerson(ctx context.Context, t *testing.T, store *adapters.PersonStore, ws, name string) domain.Person {
	t.Helper()
	p := domain.NewPerson(name, prov.Provenance{Source: "api", CapturedBy: "human:merge-test"})
	p.WorkspaceID = ws
...
func TestPersonMergeSelfMergeRejected(t *testing.T) {
	...
	if _, err := store.Merge(ctx, p.ID, p.ID, ws); !errors.Is(err, adapters.ErrSelfMerge) {
