...
func seedRecordGrantDealFixtures(ctx context.Context, t *testing.T, db *sql.DB, ws, dealName string) (ownerID, subjectID string, deal deals.Deal) {
	t.Helper()
...
func TestRecordGrantStore_CreateIdempotentUpsert(t *testing.T) {
	...
	g1, err := store.Create(ctx, adapters.CreateRecordGrantInput{
	...
	g2, err := store.Create(ctx, adapters.CreateRecordGrantInput{
		WorkspaceID: ws, GrantedBy: ownerID, RecordType: "deal", RecordID: deal.ID,
...
func TestRecordGrantStore_RejectsScopeExceedingGrant(t *testing.T) {
	...
	_, err := store.Create(ctx, adapters.CreateRecordGrantInput{
		WorkspaceID:      ws,
	...
	if !errors.Is(err, adapters.ErrGrantExceedsGrantorAccess) {
...
func TestRecordGrantStore_RevokeRemovesRow(t *testing.T) {
	...
	store := adapters.NewRecordGrantStore(db)
	g, err := store.Create(ctx, adapters.CreateRecordGrantInput{
		WorkspaceID: ws, GrantedBy: ownerID, RecordType: "deal", RecordID: deal.ID,
		SubjectType: "user", SubjectID: subjectID, Access: "read", GrantorOwnAccess: "write",
	})
	if err != nil {
