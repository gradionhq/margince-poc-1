...
func TestQuotaHandler_Create_201(t *testing.T) {
	...
	body := `{"owner_id":"owner-1","period_start":"2025-01-01","period_end":"2025-12-31","target_minor":1000000,"currency":"EUR"}`
	r := withWorkspace(httptest.NewRequest(http.MethodPost, "/quotas", strings.NewReader(body)))
...
func TestQuotaHandler_Create_422_OwnerXorTeam(t *testing.T) {
	...
	body := `{"owner_id":"o","team_id":"t","period_start":"2025-01-01","period_end":"2025-12-31","target_minor":1,"currency":"EUR"}`
	r := withWorkspace(httptest.NewRequest(http.MethodPost, "/quotas", strings.NewReader(body)))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
...
func TestQuotaHandler_Create_400_MalformedDate(t *testing.T) {
	store := &fakeQuotaStore{}
	body := `{"owner_id":"o","period_start":"not-a-date","period_end":"2025-12-31","target_minor":1,"currency":"EUR"}`
	r := withWorkspace(httptest.NewRequest(http.MethodPost, "/quotas", strings.NewReader(body)))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
...
func TestQuotaHandler_Get_200(t *testing.T) {
	...
	r := withWorkspace(httptest.NewRequest(http.MethodGet, "/quotas/q-1", nil))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
...
func TestQuotaHandler_Get_404(t *testing.T) {
	...
	r := withWorkspace(httptest.NewRequest(http.MethodGet, "/quotas/missing", nil))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
...
func TestQuotaHandler_List_200(t *testing.T) {
	...
	r := withWorkspace(httptest.NewRequest(http.MethodGet, "/quotas", nil))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
	if w.Code != http.StatusOK {
		t.Fatalf("want 200 got %d", w.Code)
...
func TestQuotaHandler_Update_200(t *testing.T) {
	...
	body := `{"currency":"EUR"}`
	r := withWorkspace(httptest.NewRequest(http.MethodPatch, "/quotas/q-1", strings.NewReader(body)))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
	if w.Code != http.StatusOK {
		t.Fatalf("want 200 got %d", w.Code)
...
func TestQuotaHandler_Update_409_VersionSkew(t *testing.T) {
	...
	if resp["code"] != "version_skew" {
		t.Fatalf("want version_skew code, got %v", resp["code"])
	}
}
...
func TestQuotaHandler_Archive_200(t *testing.T) {
	...
	r := withWorkspace(httptest.NewRequest(http.MethodDelete, "/quotas/q-1", nil))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
	if w.Code != http.StatusOK {
		t.Fatalf("want 200 got %d", w.Code)
...
func TestQuotaHandler_Attainment_200(t *testing.T) {
	...
	r := withWorkspace(httptest.NewRequest(http.MethodGet, "/quotas/q-1/attainment", nil))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
	if w.Code != http.StatusOK {
		t.Fatalf("want 200 got %d", w.Code)
...
func TestQuotaHandler_Attainment_404(t *testing.T) {
	...
	r := withWorkspace(httptest.NewRequest(http.MethodGet, "/quotas/missing/attainment", nil))
	w := httptest.NewRecorder()
	NewQuotaHandler(store).ServeHTTP(w, r)
	if w.Code != http.StatusNotFound {
		t.Fatalf("want 404 got %d", w.Code)
...
func TestQuotaHandler_Attainment_422_TargetZero(t *testing.T) {
	...
	if resp["code"] != "attainment_target_zero" {
		t.Fatalf("want attainment_target_zero got %v", resp["code"])
	}
}
...
func TestQuotaHandler_Attainment_422_ComputationFailed(t *testing.T) {
	...
	if resp["code"] != "attainment_computation_failed" {
		t.Fatalf("want attainment_computation_failed got %v", resp["code"])
	}
}
