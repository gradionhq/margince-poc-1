...
func buildAllOperations(k *routeKit) *server.AllOperations {
	...
	ops := server.NewAllOperations(
		...
		*server.NewIdentityAdapter(k.db, k.sessionStore, k.passportStore),
		...
		server.OffersAdapter{H: offerstransport.NewOfferHandler(offers.NewOfferStore(k.db).WithDealStore(deals.NewDealStore(k.db)), offers.NewOfferLineItemStore(k.db, offers.NewProductStore(k.db)), k.verifier, k.blob, offerstransport.NewNoOpRetriever())},
	)
