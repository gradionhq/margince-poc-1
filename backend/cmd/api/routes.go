...
func (k *routeKit) registerCoreCRUD(mux *http.ServeMux) {
	...
	activityHandler := actstransport.NewActivityHandler(activities.NewActivityStore(k.db))
	crud("/activities", platformauth.ObjActivity, activityHandler)
	mux.Handle("POST /activities/{id}/relink", instrument("/activities/relink", k.domainWrap(platformauth.ObjActivity, activityHandler)))
