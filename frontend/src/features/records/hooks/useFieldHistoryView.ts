    ...
    () =>
      fieldKeys.map((f) => {
        ...
        const visibleEntries =
          actor === "all" ? allEntries : allEntries.filter((e) => e.actor_type === actor);
        return {
          field: f,
  ...
  const filteredGroups = useMemo(
    () =>
      groups.filter((g) => {
        ...
        if (actor !== "all" && g.allEntries.length > 0 && g.visibleEntries.length === 0) {
          return false;
        }
        if (search && !g.label.toLowerCase().includes(search.trim().toLowerCase())) return false;
        return true;
      }),
