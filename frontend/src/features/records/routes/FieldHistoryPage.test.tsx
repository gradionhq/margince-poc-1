...
describe("FieldHistoryPage", () => {
  ...
  it("AC-field-history-1: header reads 'N fields · M changes' and the source-of-truth note", async () => {
    (apiClient.GET as ReturnType<typeof vi.fn>).mockImplementation(
      async (path: string) => {
        if (path === "/field-history")
          return {
            data: { data: ENTRIES, page: { next_cursor: null } },
            error: undefined,
            response: { status: 200 },
          };
    ...
    render(<FieldHistoryPage />, { wrapper: wrapper() });
    await waitFor(() =>
      expect(screen.getByText("5 fields · 2 changes")).toBeInTheDocument(),
    );
    expect(
      screen.getByText(/reconstructed from the append-only audit log/i),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/read-only projection, not editable here/i),
    ).toBeInTheDocument();
  });
  ...
  it("STATE-3: a field-history fetch failure shows the honest error card, never a partial timeline", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getByText(/couldn't load the change history/i),
        ).toBeInTheDocument(),
      { timeout: 5000 },
    );
  });
  ...
  it("STATE-4: a 403 on the field-history fetch shows the distinct no-access card, not the generic error", async () => {
    ...
    expect(
      screen.queryByText(/couldn't load the change history/i),
  ...
  it("AC-field-history-3/4/5: filtering and Clear filters compose end-to-end", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByTestId("field-history-group-amount_minor"),
      ).toBeInTheDocument(),
    );
    await userEvent.click(screen.getByRole("radio", { name: /^agent$/i }));
    expect(
      screen.queryByTestId("field-history-group-stage_id"),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByText(/no changes match this filter/i),
    ).toBeInTheDocument();
    await userEvent.click(
      screen.getByRole("button", { name: /clear filters/i }),
    );
    expect(
      screen.getByTestId("field-history-group-stage_id"),
    ).toBeInTheDocument();
  });
});
