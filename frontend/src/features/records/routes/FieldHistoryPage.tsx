...
export function FieldHistoryPage() {
  const { entityType: rawEntityType, entityId } = useParams<{
    entityType: string;
    entityId: string;
  }>();
  ...
  const currency =
    typeof recordQuery.data?.currency === "string"
      ? (recordQuery.data.currency as string)
      : "EUR";
  const fieldOptions = view.groups.map((g) => ({
    field: g.field,
    label: g.label,
  }));
  ...
  if (!entityType || !entityId) {
    return (
      <p className="p-gf-lg text-gf-body text-gf-secondary">Unknown record.</p>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gf-page">
      <header className="px-gf-lg py-gf-md border-b border-gf-subtle bg-gf-card">
        <div className="flex items-center gap-gf-sm">
          ...
          {!isLoading && !isForbidden && !isError && (
            <span className="ml-auto text-gf-caption text-gf-secondary">
              {view.header.fieldCount} fields · {view.header.changeCount}{" "}
              changes
            </span>
          )}
        </div>
        <p className="mt-gf-sm text-gf-caption text-gf-secondary">
          ...
          <b className="text-gf-primary">
            read-only projection, not editable here
          </b>
          .
        </p>
      </header>
      <main className="p-gf-lg">
        ...
        {isLoading ? (
          <div
            data-testid="field-history-skeleton"
            className="mt-gf-md flex flex-col gap-gf-sm"
          >
          ...
          <div className="mt-gf-md text-gf-body text-gf-status-danger">
            ...
            <p className="mt-gf-xs text-gf-caption text-gf-secondary">
              This is a read of an append-only log — your data is intact and
              nothing was lost. We won't show a partial or guessed history.
            </p>
          </div>
          ...
          <div className="mt-gf-md text-center">
            ...
            <p className="mt-gf-xs text-gf-caption text-gf-secondary">
              No field matched your search, or no change in the selected scope
              was made by that actor.
            </p>
          </div>
        ) : (
