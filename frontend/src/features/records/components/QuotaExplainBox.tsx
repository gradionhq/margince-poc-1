...
export function QuotaExplainBox({ attainment }: { attainment: QuotaAttainment }) {
  ...
  return (
    <div className="px-gf-lg pb-gf-lg">
      ...
      {open && (
        ...
        >
          <p>attainment = Σ(closed-won base_value) ÷ target, calculated to the cent</p>
          <p className="mt-gf-xs">
            closed-won ={" "}
            {attainment.contributing_deals
              .map((d) => formatMoneyDeDE(d.base_value_minor, attainment.currency))
              .join(" + ")}
          </p>
          <p className="mt-gf-xs">
            ={" "}
            ...
            ({attainment.contributing_deals.length} deals, close_date in this period)
          </p>
          <p className="mt-gf-xs">
            target ={" "}
            <b className="text-gf-primary">
          ...
          <p className="mt-gf-xs">
            attainment = {formatMoneyDeDE(attainment.closed_won_minor, attainment.currency)} ÷{" "}
            {formatMoneyDeDE(attainment.target_minor, attainment.currency)} ={" "}
            <b className="text-gf-primary">{Math.round(attainment.attainment_pct)}%</b>
          </p>
