...
describe("FieldHistoryGroupCard", () => {
  ...
  it("AC-field-history-8: zero entries shows the honest never-changed message, not a blank timeline", () => {
    render(
      <FieldHistoryGroupCard
        group={group({ allEntries: [], visibleEntries: [] })}
        currency="EUR"
      />,
    );
    expect(
    ...
    expect(
      screen.getByText(/an empty history is honest, not a gap/i),
    ).toBeInTheDocument();
  });
  ...
  it("AC-field-history-6: an agent row with evidence shows an evidence toggle; human rows never do", () => {
    ...
    const g = group({
      allEntries: [agentEntry, humanEntry],
      visibleEntries: [agentEntry, humanEntry],
    });
    render(<FieldHistoryGroupCard group={g} currency="EUR" />);
    expect(screen.getAllByRole("button", { name: /evidence/i })).toHaveLength(
      1,
    );
  });
