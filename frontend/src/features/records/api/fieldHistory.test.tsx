...
const ENTRY_BASE = {
  ...
  actor_id: "a1",
  passport_id: "psp1",
  evidence: { quote: "offer accepted", confidence: "high" },
};

describe("useFieldHistory", () => {
  it("calls GET /field-history with entity_type/entity_id/limit:200", async () => {
    (apiClient.GET as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      data: { data: [ENTRY_BASE], page: { next_cursor: null } },
      error: undefined,
      response: { status: 200 },
    });
    const { result } = renderHook(() => useFieldHistory("deal", "d1"), {
      wrapper,
    });
...
describe("useEntityRecord", () => {
  it.each([
    ...
    const { result } = renderHook(() => useEntityRecord(entityType, "x1"), {
      wrapper,
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(apiClient.GET).toHaveBeenCalledWith(
      path,
      expect.objectContaining({ params: { path: { id: "x1" } } }),
    );
  });
  ...
  it("STATE-4: 403 surfaces FieldHistoryForbiddenError (same sentinel as useFieldHistory)", async () => {
    ...
    const { result } = renderHook(() => useEntityRecord("deal", "d1"), {
      wrapper,
    });
    await waitFor(() => expect(result.current.isError).toBe(true));
    expect(result.current.error).toBeInstanceOf(FieldHistoryForbiddenError);
...
describe("shouldRetryFieldHistory", () => {
  it("never retries FieldHistoryForbiddenError", () => {
    expect(shouldRetryFieldHistory(0, new FieldHistoryForbiddenError())).toBe(
      false,
    );
  });
...
describe("scalarFieldKeys / isDiffableFieldValue", () => {
  it("excludes system metadata and object/array-valued keys, keeps scalars", () => {
    ...
    expect(keys).toEqual(
      expect.arrayContaining(["name", "amount_minor", "owner_id"]),
    );
    expect(keys).not.toEqual(
      expect.arrayContaining(["id", "workspace_id", "domains", "meta"]),
    );
  });
...
describe("computeNetTaxGross", () => {
  it("AC-field-history-7's exact worked example: 17707200 -> net 14880000, tax 2827200", () => {
    expect(computeNetTaxGross(17707200)).toEqual({
      netMinor: 14880000,
      taxMinor: 2827200,
    });
  });
...
describe("formatCurrentFieldValue / formatDiffFieldValue", () => {
  it("formats the computed money field via formatMoneyDeDE, others as plain strings", () => {
    expect(
      formatCurrentFieldValue(COMPUTED_MONEY_FIELD, 17707200, "EUR"),
    ).toMatch(/177\.072,00/);
    expect(formatCurrentFieldValue("stage_id", "Proposal sent", "EUR")).toBe(
      "Proposal sent",
    );
  });
  ...
  it("diff-token money field reparses the raw stringified minor units and reformats", () => {
    expect(
      formatDiffFieldValue(COMPUTED_MONEY_FIELD, "17707200", "EUR"),
    ).toMatch(/177\.072,00/);
  });
  it("diff-token non-money field renders the raw API string verbatim (never reformatted/guessed)", () => {
    expect(formatDiffFieldValue("stage_id", "Discovery", "EUR")).toBe(
      "Discovery",
    );
  });
});
