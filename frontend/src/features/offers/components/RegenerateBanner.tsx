...
export function RegenerateBanner({
  ...
  dealId: string;
  offer: Pick<Offer, "id" | "offer_number" | "revision" | "status" | "currency">;
  ...
  const counts = response?.diff_from_previous ? diffCounts(response.diff_from_previous) : null;
  ...
  return (
    <section className="rounded-gf-lg border border-gf-subtle bg-gf-card p-gf-lg">
      <div className="flex flex-wrap items-center justify-between gap-gf-sm">
        <div>
          <h2 className="text-gf-body font-medium text-gf-primary">Regenerate</h2>
                    ...
                    <tbody>
                      ...
                      {diff.changed?.map(({ before, after }, index) => (
                        <tr key={after?.id ?? index}>
                          ...
                          <td>
                            {before?.unit_price_minor} → {after?.unit_price_minor}
                          </td>
                          <td>
                          ...
                          <td>
                            <LineProvenanceBadge
                              ...
                              capturedBy={after?.captured_by ?? before?.captured_by ?? ""}
                              evidence={after?.evidence ?? before?.evidence ?? null}
                            />
                          </td>
                        </tr>
                      ))}
