  ...
  return (
    <div className="flex flex-col gap-gf-sm">
      {notes.length === 0 ? (
        ...
        <ul className="flex flex-col gap-gf-xs">
          {notes.map((n) => (
            <li
              key={n.id}
              className="flex items-center gap-gf-xs border border-gf-subtle rounded-md p-gf-sm"
            >
      ...
      <p className="text-gf-label text-gf-secondary italic">
        No notes-create endpoint exists in the contract yet — this is not yet
        persisted to the backend; it lives in local component state only.
      </p>
    </div>
  );
}
