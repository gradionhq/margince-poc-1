...
derives-from:
  ...
  - margince specs/spec/architecture/04-seam-evolution.md#0-the-one-constraint-everything-follows-from
  - margince specs/spec/architecture/04-seam-evolution.md#1-safe-vs-breaking-per-seam-type
  - margince specs/spec/architecture/04-seam-evolution.md#2-the-additive-evolution-mechanism-the-primary-tool
  - margince specs/spec/architecture/04-seam-evolution.md#3-classification-every-exported-type-is-seam-or-sealed-internal
  - margince specs/spec/architecture/04-seam-evolution.md#4-openapi--http-contract-evolution-stripe-style-oasdiff-gated
---
...
from the detector rather than from memory (CP-BREAK-25).
...
| CP-STAGE-7 | Breaking-change gate (merge-blocking) | `oasdiff breaking` between the base branch's contract and the working copy; severity→policy is `--fail-on ERR` (CP-BREAK-20). Skips gracefully but loudly (prints why) when the tool or a base ref is unavailable — same degrade pattern as the drift gate. |
